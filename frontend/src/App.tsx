...
const DEGRADED_NOTE: Record<string, string> = {
  ffmpeg:
    "ffmpeg is absent, so the decode-error and quality detectors do not run.",
  ffprobe:
    "ffprobe is absent, so the decode-error and quality detectors do not run.",
  playwright:
    "Playwright is absent, so PDF export is unavailable. HTML export works.",
  database:
    "The database does not answer, so jobs and reports are not persisted.",
};
...
export default function App() {
  ...
  const [prefill, setPrefill] = useState<
    Record<"realtime" | "aging", ChannelPrefill | null>
  >({
    realtime: null,
    aging: null,
  });
   ...
   */
  const checks =
    (health?.checks as Record<string, { installed?: boolean }> | undefined) ??
    {};
  ...
  const railHealth = {
    ...
    detail: [
      degraded.length === 0
        ...
        : degraded
            .map((item) => DEGRADED_NOTE[item] ?? `${item} is unavailable.`)
            .join(" "),
      noBento4
        ? "mp4decrypt is not installed, so cbcs tracks are not decrypted."
        : "",
    ]
      .filter(Boolean)
      .join(" "),
