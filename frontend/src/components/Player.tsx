...
export function Player({ src, onEvent, drm }: Props) {
  ...
  useEffect(() => {
    ...
    if (protectedStream && !emeAvailable) {
      ...
      onEvent({
        event: "drm",
        details: "secure_context_missing",
        error_type: reason,
      });
      return () => {
        cleanup();
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Events.LEVEL_SWITCHED, (_e, data: LevelSwitchedData) => {
        ...
        currentVariant = level
          ? `v${level.height}p@${Math.round(level.bitrate / 1000)}k`
          : "";
      ...
      if (protectedStream) {
        ...
        hls.on(Events.FRAG_DECRYPTED, () => {
          if (decryptedRef.current === 0) {
            onEvent({
              event: "drm",
              details: "first_fragment_decrypted",
              variant: currentVariant,
            });
          }
      ...
      hls.on(Events.ERROR, (_e, data: ErrorData) => {
        ...
        const explained = explainDrm(
          String(data.details),
          drm?.system_label ?? "the key system",
        );
      ...
      setError(
        "This browser cannot play HLS natively and hls.js reports no support.",
      );
    }
    ...
    const onPlaying = () => {
      if (startedAtRef.current) {
        onEvent({
          event: "startup",
          startup_ms: performance.now() - startedAtRef.current,
        });
    ...
    <div className="card overflow-hidden">
      ...
      {src && drm?.protected && !drm.configured && (
        <p className="border-t border-violet-200 bg-violet-50 px-4 py-2.5 text-small text-violet-700">
          This channel is protected with {drm.system_label} and no licence URL
          is configured, so the picture will not start. Set it in Settings
          &rarr; DRM. The analysis runs either way: the segments are decrypted
          on the backend.
        </p>
      )}
      {!src && (
