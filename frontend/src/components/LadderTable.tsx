...
function Cell({
  ...
  const differs =
    declared != null &&
    measured != null &&
    String(declared) !== String(measured);
  return (
    <td className={cx(differs && "bg-pink-50")}>
      <div
        className={cx(
          "font-mono text-micro",
          differs ? "text-pink-600" : "text-ink",
        )}
      >
        {shown(declared)}
      </div>
      {measured !== undefined && (
        <div
          className={cx(
            "font-mono text-[11px]",
            differs ? "text-pink-600" : "text-ink-faint",
          )}
        >
          {shown(measured)}
        </div>
    ...
    <div>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={`${row.layer}:${row.variant}`}>
                ...
                <Cell
                  declared={kbps(row.declared.bandwidth)}
                  measured={undefined}
                />
                <Cell
                  declared={row.declared.resolution}
                  measured={row.measured.resolution}
                />
                <Cell
                ...
                />
                <Cell
                  declared={row.declared.codecs}
                  measured={row.measured.audio_codec}
                />
                <Cell
                ...
                <Cell
                  declared={
                    row.measured.peak_kbps
                      ? `${Math.round(row.measured.peak_kbps)} kbit/s`
                      : "—"
                  }
                  measured={undefined}
                />
      ...
      {refFramesDiffer && (
        <p className="border-t border-pink-200 bg-pink-50 px-5 py-2.5 text-small text-pink-600">
          The rungs declare different reference frame counts. Every ABR switch
          between them reallocates the decoded picture buffer.
        </p>
      )}
      <p className="px-5 py-2.5 text-micro text-ink-muted">
        The upper value in each cell is the manifest declaration; the lower
        value is what the bitstream carries. A rung marked{" "}
        <span className="font-mono">demuxed</span> takes its audio from the
        rendition named beneath it, so its own segments carry video only.
      </p>
    </div>
  );
}
