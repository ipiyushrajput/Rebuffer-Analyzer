...
export interface DrmSettings {
  ...
  /** Where the browser sends the licence challenge: through this analyzer, or straight to the licence server. */
  license_request_path: "relay" | "direct";
  configured: boolean;
  endpoint: string;
...
function cascadaChannelQuery(
  params: CascadaChannelQuery & { refresh?: boolean },
): string {
...
export const endpoints = {
  ...
  createRealtime: (body: unknown) =>
    api.post<RealtimeSession>("/realtime/sessions", body),

  drmSettings: () =>
    api.get<{ drm: DrmSettings; endpoint_default: string }>("/drm/settings"),
  saveDrmSettings: (body: unknown) =>
    api.put<{ drm: DrmSettings }>("/drm/settings", body),
  drmProbe: (url: string) =>
  ...
  realtimeResult: (id: string) =>
    api.get<Record<string, unknown>>(`/realtime/sessions/${id}/result`),
  ...
  listAging: () =>
    api.get<{ jobs: JobSummary[]; stored_error?: string | null }>(
      "/aging/jobs",
    ),
  ...
  readBulk: (id: string) =>
    api.get<JobSummary & { items: Record<string, unknown>[] }>(
      `/bulk/jobs/${id}`,
    ),
  ...
  saveCascadaSession: (body: {
    cookie_header?: string;
    sessionid?: string;
    csrftoken?: string;
  }) => api.put<CascadaSessionState>("/cascada/session", body),
  validateCascadaSession: () =>
    api.post<CascadaSessionState>("/cascada/session/validate"),
  ...
  cancelCascadaScan: (id: string) =>
    api.del<CascadaScan>(`/cascada/scans/${id}`),
  ...
  saveBatchSettings: (body: BatchSettings) =>
    api.put<{ settings: BatchSettings; defaults: BatchSettings }>(
      "/batch/settings",
      body,
    ),
  batchSchedules: () =>
    api.get<{ schedules: BatchSchedule[]; min_days_between_runs: number }>(
      "/batch/schedules",
    ),
  ...
  deleteBatchSchedule: (country: string) =>
    api.del<{ deleted: boolean }>(
      `/batch/schedules/${encodeURIComponent(country)}`,
    ),

  batchEstimate: (country: string) =>
    api.get<BatchEstimate>(
      `/batch/estimate?country=${encodeURIComponent(country)}`,
    ),
  startBatch: (country: string) =>
    api.post<Batch>("/batch/batches", { country }),
  listBatches: () =>
    api.get<{ batches: Batch[]; count: number }>("/batch/batches"),
  ...
  batchLog: (id: string) =>
    api.get<{ lines: BatchLogLine[] }>(`/batch/batches/${id}/log`),
  batchLogUrl: (id: string) =>
    api.url(`/batch/batches/${id}/log?download=true`),
  batchReportUrl: (id: string, fmt: "csv" | "xlsx") =>
    api.url(`/batch/batches/${id}/report.${fmt}`),
  batchColumns: () =>
    api.get<{ columns: string[]; window_days: number; window_label: string }>(
      "/batch/columns",
    ),

  agingSamples: (
    id: string,
    params: { from?: string; to?: string; maxPoints?: number } = {},
  ) => {
    const query = new URLSearchParams();
    ...
    return api.get<JobSamples>(
      `/aging/jobs/${id}/samples${suffix ? `?${suffix}` : ""}`,
    );
  },

  listChannels: () =>
    api.get<{ channels: AnalysedChannel[]; count: number }>("/channels"),
  ...
  listReports: (query = "") =>
    api.get<{ reports: Record<string, unknown>[] }>(`/reports${query}`),
  ...
  saveSettings: (body: unknown) =>
    api.put<Record<string, unknown>>("/settings", body),
  ...
  setRuleSeverity: (ruleId: string, severity: string | null) =>
    api.put<RuleCatalogue>(`/settings/rules/${encodeURIComponent(ruleId)}`, {
      severity,
    }),
};
